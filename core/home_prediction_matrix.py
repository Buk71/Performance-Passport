...
The matrix is deliberately a translation, not another coach vote. It uses
the same Riegel 1.06 race-equivalence rule already used by Race Coach and
retains the athlete's existing condition multipliers from Home Predictions.
...
def build_home_prediction_matrix(
    ...
    return HomePredictionMatrix(
        ...
        explanation=(
            ...
            "active-goal capability using Race Coach's 1.06 equivalence rule. "
            "They describe capability, not distance-specific race readiness."
        ),
    )
