...
The audit deliberately does not replace ``classify_session``.  It compares the
live classifier with stricter physical lap evidence, exposes disagreement, and
builds a real-activity reference set before any coaching model is changed.
"""
...
AUDIT_MODEL_VERSION = "recognition-audit-v2"
...
def _stopped_watch_repetitions(
    ...
    # A stopped watch does not turn ordinary kilometre auto-laps into work
    # repetitions. Without distinct boundaries or recovery laps there is no
    # defensible way to separate a steady interrupted run from 1 km intervals.
    # Credible 1 km workouts remain eligible through the normal recovery and
    # boundary-based evidence routes instead.
    kilometre_auto_laps = sum(
        0.98 <= item.distance_km <= 1.02 for item in family
    )
...
def _session_kind(facts: ActivityFacts, session: Any) -> str:
    ...
    if intent in {"easy_with_strides", "easy_with_pickups", "standalone_strides"}:
        return str(intent)
